...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { company, position, period, description } = await req.json();

    await pool.query(
      'UPDATE experience SET company = ?, position = ?, period = ?, description = ? WHERE id = ?',
      [company, position, period, description, id]
    );
